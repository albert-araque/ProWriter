	...
	public void initialize(URL location, ResourceBundle resources) {	
		...
		addProjectButton.setOnMouseClicked(new EventHandler<Event>() {
			...
			public void handle(Event event) {
				...
				addProjectDialog.initOwner(Main.mainStage);                
		...
		updateProjectButton.setOnMouseClicked(new EventHandler<Event>() {
			...
			public void handle(Event event) {
				...
				updateProjectDialog.initOwner(Main.mainStage);
...
//				updateProjectPane(updateProjectDialog.showAndReturn(updateController));
		...
		projectPane.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if (event.getClickCount() == 1) {
					selectedProject = p;
//					selectedPane = null;
//					selectedPane = projectPane;
					
//					selectedPane = getSelectedPane();			
					
//					selectedPaneLabel.setText(selectedPane.toString());
	...
	//De la manera que estaba hecho habia un bug que no actualizaba correctamente los proyectos al hacer un update, se queda de la manera en la que esta
	//y si tengo tiempo mirare de arreglarlo porque como esta hecho actualmente cada vez que actualiza hace una consulta a la base de datos para coger
	//todos los proyectos y los vuelve a poner y es mucho mas lento
	
//	private Pane getSelectedPane() {
//		
//		for (Pane pane : projectPanes) {
//			if (pane.equals(projectPane)) return pane;
//		}
//		
//		return null;
//	}	
	//actualiza el contenedor de proyecto, borra el que habia previamente y añade otro con la informacion actualizada, pone las variables a null para deseleccionarlo todo
//	private void updateProjectPane(Proyecto p) {
//		
//		projectFlowPane.getChildren().remove(selectedPane);
//		
//		selectedPane = null;
//		selectedProject = null;
//		convertProjectToPane(p);
//		selectedPaneLabel.setText("Ningun pane seleccionado");
//		selectedProjectLabel.setText("Ningun proyecto seleccionado");
//	}
